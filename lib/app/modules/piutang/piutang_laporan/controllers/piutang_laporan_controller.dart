...
class PiutangLaporanController extends GetxController {
  ...
  Future<void> loadLaporanData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${BaseUrl.baseUrl}/laporan-piutang'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  List<pw.Widget> _buildFlatWidgetList(
    ...
    final daftarPiutang = laporanData['daftar_piutang'] as List<dynamic>? ?? [];
    ...
    return [
      ...
      // Langsung return TableHelper tanpa wrapper widget
      pw.Text(
        'Daftar Piutang',
      ...
      pw.TableHelper.fromTextArray(
        ...
        headers: ['Nama', 'Deskripsi', 'Total Piutang', 'Total Cicilan', 'Sisa Piutang'],
        data: daftarPiutang
            .map<List<dynamic>>((item) => [
                  item['nama_debitur']?.toString() ?? '',
                  '${item['deskripsi']?.toString() ?? ''}\nTanggal Piutang: ${item['tanggal_piutang']?.toString() ?? ''}\nJatuh Tempo: ${item['tanggal_jatuh_tempo']?.toString() ?? ''}',
                  item['total_piutang']?.toString() ?? '',
                  item['total_cicilan']?.toString() ?? '',
                  item['sisa_piutang']?.toString() ?? '',
                ])
            .toList(),
      ),
    ];
          ...
          children: [
            pw.TableRow(children: [
              ...
              pw.Container(
                ...
                child: pw.Text(
                  laporanData['total_piutang']?.toString() ?? '0',
                  style: pw.TextStyle(
                    font: ttfBold,
                    fontSize: 9,
                    fontWeight: pw.FontWeight.bold,
  ...
  String _generateFileName() {
    ...
    return 'Laporan_Piutang_$dateStr.pdf';
  }
  ...
  Future<void> _sharePDF(String filePath) async {
    try {
      ...
      String shareText = 'Laporan Piutang - ${_getReportSubtitle()}';
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Laporan piutang telah berhasil diekspor ke PDF',
                    style:
