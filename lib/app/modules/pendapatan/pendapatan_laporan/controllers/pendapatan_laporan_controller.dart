...
class PendapatanLaporanController extends GetxController {
  ...
  String _getEndpoint() {
    switch (selectedPeriod.value) {
      ...
      case ReportPeriod.weekly:
        return '/laporan-tujuh-hari-terakhir';
      case ReportPeriod.monthly:
        return '/laporan-pendapatan-bulanan';
      case ReportPeriod.custom:
        return '/laporan-pendapatan-custom';
    }
  }
  ...
  //           if (pageNumber == 1) ...[
  //             _buildKopSurat(
  //                 ttfBold, ttf, masjidName, title, subtitle, logoImage),
  //             pw.SizedBox(height: 15),
  //             _buildSummarySection(ttfBold, ttf),
  //             pw.SizedBox(height: 25),
  //             if (laporanData['kategori_pendapatan'] != null) ...[
  //               _buildKategoriTable(ttfBold, ttf),
  //               pw.SizedBox(height: 25),
  //             ],
  //           ],

  //           // Header untuk halaman lanjutan
  //           if (pageNumber > 1) ...[
  //             pw.Text(
  //               '$title (Lanjutan)',
  ...
  //               children: [
  //                 pw.Text(
  //                   'Dicetak: ${DateFormat('dd/MM/yyyy HH:mm', 'id').format(DateTime.now())}',
            ...
            pw.TableRow(children: [
              ...
              pw.Container(
                ...
                alignment:
                    pw.Alignment.center,
                child: pw.Text(
                  _formatCurrency(laporanData['total_pendapatan']),
                  style: pw.TextStyle(
                    font: ttfBold,
                    fontSize: 9,
                    fontWeight: pw.FontWeight.bold,
 ...
 pw.Widget _buildKategoriTable(pw.Font ttfBold, pw.Font ttf) {
  final kategoriData =
      laporanData['kategori_pendapatan'] as List<dynamic>? ?? [];

  return pw.Column(
    crossAxisAlignment: pw.CrossAxisAlignment.start,
    children: [
      pw.Text('Kategori Pendapatan',
          style: pw.TextStyle(
              font: ttfBold, fontSize: 11, color: PdfColors.black)),
      pw.SizedBox(height: 10),
      pw.Table(
        ...
        children: [
          ...
          ...kategoriData.map<pw.TableRow>((kategori) => pw.TableRow(children: [
                pw.Container(
                  padding: const pw.EdgeInsets.all(8),
                  alignment: pw.Alignment.centerLeft,
                  child: pw.Text(
  ...
  // pw.Widget _buildSummaryPerHariTable(pw.Font ttfBold, pw.Font ttf) {
  //   final summaryData = laporanData['summary_per_hari'] as List<dynamic>? ?? [];

  //   return pw.Column(
  //       crossAxisAlignment: pw.CrossAxisAlignment.start,
  //       children: [
  //         pw.Text('RINGKASAN PER HARI',
  //             style: pw.TextStyle(
  //                 font: ttfBold, fontSize: 14, color: PdfColors.black)),
  //         pw.SizedBox(height: 10),
  //         pw.Table(
  //           border: pw.TableBorder.all(color: PdfColors.black),
  //           children: [
  //             _buildTableHeader(['Tanggal', 'Total'], ttfBold),
  //             ...summaryData.map<pw.TableRow>((hari) => pw.TableRow(children: [
  //                   _buildTableCell(hari['tanggal'] ?? '', ttf),
  //                   _buildTableCell(hari['total'] ?? 'Rp 0', ttf),
  //                 ])),
  //           ],
  //         ),
  //       ]);
  ...
  //     children: [
  //       pw.Text(
  //         'Riwayat Transaksi (${riwayatData.length} transaksi)',
  //         style:
  //             pw.TextStyle(font: ttfBold, fontSize: 11, color: PdfColors.black),
  //       ),
  //       pw.SizedBox(height: 10),
  ...
  //     decoration: pw.BoxDecoration(color: PdfColor.fromHex('#F8F9FA')),
  //     children: headers
  //         .map((header) => _buildTableCell(header, ttfBold, isHeader: true))
  //         .toList(),
  //   );
  // }
  ...
  //       children: [
  //         pw.Text(
  //             'Laporan dibuat pada: ${DateFormat('dd MMMM yyyy HH:mm', 'id_ID').format(DateTime.now())}',
  //             style: pw.TextStyle(
  //                 font: ttf, fontSize: 10, color: PdfColor.fromHex('#666666')),
  //             textAlign: pw.TextAlign.center),
  //         pw.SizedBox(height: 5),
  //         pw.Text('Sistem Manajemen Masjid eMasjid Pro',
  //             style: pw.TextStyle(
  //                 font: ttf, fontSize: 10, color: PdfColor.fromHex('#666666')),
  //             textAlign: pw.TextAlign.center),
  //       ],
  ...
  String _generateFileName() {
    ...
    switch (selectedPeriod.value) {
      case ReportPeriod.daily:
        return 'Laporan_Pendapatan_Harian_$dateStr.pdf';
      case ReportPeriod.weekly:
        return 'Laporan_Pendapatan_7Hari_$dateStr.pdf';
      case ReportPeriod.monthly:
        final monthName = months
            .firstWhere((m) => m['value'] == selectedMonth.value)['label'];
        return 'Laporan_Pendapatan_${monthName}_$dateStr.pdf';
      case ReportPeriod.custom:
        return 'Laporan_Pendapatan_$dateStr.pdf';
    }
  }
  ...
  Future<void> _sharePDF(String filePath) async {
    try {
      ...
      String shareText = 'Laporan Pendapatan - ${_getReportSubtitle()}';
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Laporan pendapatan telah berhasil diekspor ke PDF',
                    style:
